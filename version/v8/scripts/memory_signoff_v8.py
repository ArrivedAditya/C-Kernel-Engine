...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run v8 memory sign-off and emit memory_signoff.json")
    ...
    parser.add_argument(
        "--gate-profile",
        choices=["inference", "full"],
        default=str(os.environ.get("CK_V7_MEMORY_GATE_PROFILE", "inference")).strip().lower(),
        help="Gate profile: inference=planner+kv required (advanced optional), full=all required",
    )
    ...
    if args.gate_profile == "full":
        ...
        adv = {
            ...
            "errors": [],
            "warnings": [],
            "info": ["Skipped in inference gate profile; run --gate-profile full for deep checks."],
        }
    ...
    required_checks = ["memory_planner", "kv_cache"] if args.gate_profile == "inference" else list(checks.keys())
    passed = all(bool(checks.get(name, {}).get("passed")) for name in required_checks)
    report = {
    ...
    print(
        f"[memory-signoff] {status} ({args.gate_profile}): "
        f"errors={err_count} warnings={warn_count} required={','.join(required_checks)}"
    )
