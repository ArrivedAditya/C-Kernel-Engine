...
================================================================================
  ...
  This includes:
    - Generated C code (in ~/.cache/ck-engine-v7/ or similar)
    - codegen_v7.py - memory layout generation
    - build_ir_v7.py - weight manifest offsets
...
EXT_METADATA_SIZE = 24                               # Extended metadata block (v7+)
...
def write_bumpv5_footer(f: BinaryIO, meta_size: int, meta_sha256: bytes) -> None:
    """Append BUMPWGT5 metadata footer (EOF locator)."""
...
def load_template_for_arch(arch: str) -> dict:
    ...
    if not os.path.exists(template_path):
        raise GGUFError(
            f"Missing template for arch '{arch}'. Expected: {template_path}. "
...
def verify_bump_parity(
    ...
    with open(gguf_path, "rb") as gf, open(bump_path, "rb") as bf:
        ...
        if bump_magic not in (b"BUMPWGT4", b"BUMPWGT5"):
...
def _scan_cached_models() -> list:
    ...
    import glob
    cache_dirs = [
        os.path.expanduser("~/.cache/ck-engine-v7/models"),
...
def main() -> None:
    ...
    with open(args.gguf, "rb") as f:
        ...
        if arch == "clip" and clip_projector_type == "qwen3vl_merger":
            ...
            if missing_meta:
                raise GGUFError(
                    f"Missing required CLIP/Qwen3-VL metadata: {missing_meta}"
                )
            if patch_size <= 0 or image_size <= 0:
                raise GGUFError(
                    f"Invalid CLIP/Qwen3-VL patch/image size: patch={patch_size}, image={image_size}"
                )
            if embed_dim % num_heads != 0:
                raise GGUFError(
                    f"CLIP/Qwen3-VL embed_dim {embed_dim} not divisible by num_heads {num_heads}"
                )
            ...
            deepstack_layer_indices: list[int] = []
            if isinstance(deepstack_layers, (list, tuple)):
                for idx, value in enumerate(deepstack_layers):
                    enabled = False
                    ...
                    if enabled:
                        deepstack_layer_indices.append(int(idx))
            for tensor_name in tensors.keys():
                if not tensor_name.startswith("v.deepstack."):
                    continue
                parts = tensor_name.split(".")
                if len(parts) > 2 and parts[2].isdigit():
                    deepstack_layer_indices.append(int(parts[2]))
            deepstack_layer_indices = sorted(set(deepstack_layer_indices))
            ...
            for src_name in (
                ...
                add_vision_entry(src_name, label=src_name)
            ...
            for layer in range(num_layers):
                ...
                q: Dict[str, str] = {}
                for ck_name, src_name in (
                    ("attn_qkv", f"v.blk.{layer}.attn_qkv.weight"),
                    ("wo", f"v.blk.{layer}.attn_out.weight"),
                    ("w3", f"v.blk.{layer}.ffn_up.weight"),
                    ("w2", f"v.blk.{layer}.ffn_down.weight"),
                ):
                ...
                for src_name in (
                    f"v.blk.{layer}.ln1.weight",
                    f"v.blk.{layer}.ln2.weight",
                    f"v.blk.{layer}.attn_qkv.weight",
                    f"v.blk.{layer}.attn_qkv.bias",
                    f"v.blk.{layer}.attn_out.weight",
                    f"v.blk.{layer}.attn_out.bias",
                    f"v.blk.{layer}.ffn_up.weight",
                    f"v.blk.{layer}.ffn_down.weight",
                ):
                    add_vision_entry(src_name, label=src_name)
                for src_name in (
                    f"v.blk.{layer}.ln1.bias",
                    f"v.blk.{layer}.ln2.bias",
                    f"v.blk.{layer}.ffn_up.bias",
                    f"v.blk.{layer}.ffn_down.bias",
                ):
                    maybe_add_vision_entry(src_name, label=src_name)

            quant_summary["patch_emb"] = ck_dtype_name(
                clip_weight_dtype(tensors["v.patch_embd.weight"], "v.patch_embd.weight")
            ).lower()
            quant_summary["patch_emb_aux"] = ck_dtype_name(
                clip_weight_dtype(tensors["v.patch_embd.weight.1"], "v.patch_embd.weight.1")
            ).lower()
            quant_summary["mm0_w"] = ck_dtype_name(
                clip_weight_dtype(tensors["mm.0.weight"], "mm.0.weight")
            ).lower()
            quant_summary["mm1_w"] = ck_dtype_name(
                clip_weight_dtype(tensors["mm.2.weight"], "mm.2.weight")
            ).lower()

            for layer in deepstack_layer_indices:
                deepstack_key = f"deepstack.{layer}"
                q: Dict[str, str] = {}
                for ck_name, src_name in (
                    ("norm", f"v.deepstack.{layer}.norm.weight"),
                    ("fc1", f"v.deepstack.{layer}.fc1.weight"),
                    ("fc2", f"v.deepstack.{layer}.fc2.weight"),
                ):
                    info = tensors.get(src_name)
                    if info is not None:
                        q[ck_name] = ck_dtype_name(clip_weight_dtype(info, src_name)).lower()
                if q:
                    quant_summary[deepstack_key] = q
                for src_name in (
                    f"v.deepstack.{layer}.norm.weight",
                    f"v.deepstack.{layer}.norm.bias",
                    f"v.deepstack.{layer}.fc1.weight",
                    f"v.deepstack.{layer}.fc1.bias",
                    f"v.deepstack.{layer}.fc2.weight",
                    f"v.deepstack.{layer}.fc2.bias",
                ):
                    maybe_add_vision_entry(src_name, label=src_name)

            ignored_sources = sorted(set(tensors.keys()) - consumed_sources)
            source_coverage = {
                "arch": arch,
                "conversion_arch": vision_arch,
                "projector_type": clip_projector_type,
            ...
            mm0 = tensors["mm.0.weight"]
            mm1 = tensors["mm.2.weight"]
            ...
            vision_config = _inject_runtime_config_defaults({
                ...
                "projector_out_dim": int(projector_out_dim),
                "projector_total_out_dim": int(projector_out_dim * (1 + len(deepstack_layer_indices))),
                "projection_dim": int(projection_dim),
            ...
            print(
                f"[gguf->bump] version={args.bump_version} arch={arch}->{vision_arch} layers={num_layers} "
                f"hidden={embed_dim} heads={num_heads}/{num_kv_heads} ff={intermediate} "
                f"patches={vision_num_patches} projector={clip_projector_type} -> {args.output}"
            )
            print_generic_conversion_report(
                arch=vision_arch,
                manifest_entries=manifest_entries,
            ...
            if args.manifest_out:
                ...
                manifest = manifest_dict or {
                    "version": 5,
                    "model": vision_arch,
                    "config": vision_config if args.bump_version == BUMP_VERSION_V5 else None,
            ...
            if args.bump_version == BUMP_VERSION_V5:
                ...
                for i, info in enumerate(layer_infos):
                    quant_summary[f"layer.{i}"] = {
                        ...
                        "wk": get_quant_type_name(info["wk"].ggml_type),
                        "wv": get_quant_type_name(info["wv"].ggml_type),
                        "wo": get_quant_type_name(info["wo"].ggml_type),
                ...
                out_f.write(b"BUMPWGT5")
                out_f.write(struct.pack("<I", 5))  # version
                ...
                print(f"[bumpv5] Metadata: {meta_size} bytes @ offset {meta_offset}")
                print(f"[bumpv5] Template: {template_data.get('name', arch)}, quant_summary: {len(quant_summary)} layers")
