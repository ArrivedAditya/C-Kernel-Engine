...
This is the v8 promotion lane for text-family inference:
- family-specific runtime arguments live in version/v8/regression/families.json
- prompt/coherence expectations live in version/v8/regression/prompts.json

For now this focuses on the promoted inference surface:
- build/runtime
- smoke prompts
- coherence scoring
- runtime contract/artifact audit
...
def assess_coherence(text: str, heuristics: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(metrics["required_marker_hits"]) < min_required_marker_hits:
        reasons.append(f"required_markers:{len(metrics['required_marker_hits'])}<{min_required_marker_hits}")
...
def audit_runtime_contract(
    ...
    required_stdout_any = [str(x) for x in expect.get("stdout_contains_any_of") or []]
    ...
    if required_stdout_any and not any(needle and needle in stdout for needle in required_stdout_any):
        reasons.append(f"stdout_missing_any_of:{required_stdout_any!r}")
    ...
    result = {
        ...
        "stdout_contains_any_of": required_stdout_any,
...
def render_terminal_matrix(summary: dict[str, Any]) -> str:
    rows: list[tuple[str, ...]] = [("Family", "Build", "Smoke", "Coherence", "Contract", "Status/Class")]
...
def render_markdown_report(summary: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "| Family | Build | Smoke | Coherence | Contract | Status |",
        "|---|---|---|---|---|---|",
    ]
    for family in summary.get("families", []):
        lines.append(
            "| {family_id} | {build_status} | {smoke_status} | {coherence_status} | {contract_status} | {status} |".format(
                **family
            )
        )
    for family in summary.get("families", []):
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run v8 model-family regression checks")
    ...
    print("=" * 72)
    print("C-Kernel-Engine v8 Regression")
    print("=" * 72)
