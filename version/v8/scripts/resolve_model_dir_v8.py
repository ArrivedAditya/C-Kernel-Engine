#!/usr/bin/env python3
"""
Resolve v8 model input into a concrete v8 model cache directory.

This is used by Make targets that need direct paths for v8 reports and hub
generation after a run has materialized the cached model directory.
"""

from __future__ import annotations

import argparse
from pathlib import Path

from ck_run_v8 import CACHE_DIR, detect_input_type


def resolve_model_dir(model_input: str) -> Path:
    input_type, info = detect_input_type(model_input)

    if input_type == "hf_gguf":
        return CACHE_DIR / info["repo_id"].replace("/", "--")
    if input_type == "hf_id":
        return CACHE_DIR / info["model_id"].replace("/", "--")
    if input_type == "gguf":
        return CACHE_DIR / info["path"].stem
    if input_type == "local_dir":
        local_dir = Path(info["path"]).resolve()
        if (local_dir / "weights.bump").exists() and (local_dir / "weights_manifest.json").exists():
            return local_dir
        return local_dir / ".ck_build_v8"
    if input_type == "local_config":
        return Path(info["path"]).resolve().parent

    raise ValueError(f"Unsupported model input type: {input_type}")


def main() -> int:
    parser = argparse.ArgumentParser(description="Resolve model dir for v8 input")
    parser.add_argument("--model-input", required=True, help="Same model input accepted by ck_run_v8.py")
    args = parser.parse_args()

    model_dir = resolve_model_dir(args.model_input)
    print(str(model_dir))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
