#!/usr/bin/env python3
"""
build_ir_v7.py - Complete IR Pipeline: Template + Quant → IR1 → Fusion → Layout
...
USAGE:
    # Generate IR1 only
    python build_ir_v7.py --manifest=/path/to/weights_manifest.json \\
...
PROJECT_ROOT = SCRIPT_DIR.parent  # version/v7
...
def load_kernel_registry() -> Dict:
    """Load kernel registry (v7-specific)."""
...
def build_ir1_direct(manifest: Dict, manifest_path: Path, mode: str = "decode",
    ...
    if unmapped_ops:
        ...
        for op in unmapped_ops:
            print(f"  - {op}")
        print(f"\nAction required:")
        print(f"  Add mappings to TEMPLATE_TO_KERNEL_OP in build_ir_v7.py")
