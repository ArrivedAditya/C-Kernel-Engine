        ...
        system_profiles = [
            {
                "name": "cke_perf_stat",
                "tool": "perf",
                "command": ["perf", "stat", "-x,", "-e", perf_events, "-o", str(profile_dir / "cke-perf-stat.csv"), "--", *cke_command],
            },
            {
            ...
            },
            {
                ...
                "tool": "perf",
                "command": ["perf", "record", "-F", "199", "-g", "--call-graph", "dwarf", "-o", str(profile_dir / "cke-perf.data"), "--", *cke_command],
            },
            {
                "name": "cke_vtune_hotspots",
                "tool": "vtune",
                "command": ["vtune", "-collect", "hotspots", "-result-dir", str(profile_dir / "cke-vtune-hotspots"), "-quiet", "--", *cke_command],
            },
            {
            ...
            },
            {
                ...
                "tool": "vtune",
                "command": ["vtune", "-collect", "uarch-exploration", "-result-dir", str(profile_dir / "cke-vtune-uarch"), "-quiet", "--", *cke_command],
            },
            {
                "name": "cke_vtune_memory",
                "tool": "vtune",
                "command": ["vtune", "-collect", "memory-access", "-result-dir", str(profile_dir / "cke-vtune-memory"), "-quiet", "--", *cke_command],
            },
            {
                "name": "cke_advisor_roofline",
                "tool": "advisor",
                "command": ["advisor", "--collect=roofline", "--project-dir", str(profile_dir / "cke-advisor-roofline"), "--", *cke_command],
            },
        ]
        plan.append(
            {
...
<h2>Performance matrix</h2><div class="table"><table><thead><tr><th>Model</th><th>Quant</th><th>Prompt</th><th>CKE ms</th><th>llama ms</th><th>CKE/llama</th><th>CKE decode tok/s</th><th>llama decode tok/s</th></tr></thead><tbody>{''.join(perf_body)}</tbody></table></div>
<h2>Prompt output comparison</h2>{''.join(prompt_body)}
<h2>Operation attribution</h2><div class="table"><table><thead><tr><th>Model</th><th>Prompt</th><th>Wall ms</th><th>Profiled ms</th><th>Coverage</th><th>Top operations</th></tr></thead><tbody>{''.join(operation_body)}</tbody></table></div>
...
def main() -> int:
    ...
    parser.add_argument("--contexts", default="32,128,512,1024,2048")
