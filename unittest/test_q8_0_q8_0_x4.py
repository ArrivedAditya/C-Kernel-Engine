...
def test_q8_0_m2n4_matches_established_gemm_bit_exact() -> None:
    root = Path(__file__).resolve().parents[1]
    lib = ctypes.CDLL(
        str(root / "build" / "libckernel_engine.so"),
        mode=getattr(os, "RTLD_LOCAL", 0),
    ...
    lib.gemm_nt_q8_0_q8_0.argtypes = signature
    lib.gemm_nt_q8_0_q8_0_m2n4.argtypes = signature
    lib.gemm_nt_q8_0_q8_0_m2n4_tile.argtypes = signature + [ctypes.c_int]

    rng = np.random.default_rng(0x5137)
    tokens, rows, width = 5, 19, 96
    blocks = width // 32
    weights = b"".join(
        _block(float(rng.uniform(0.001, 0.05)), rng.integers(-127, 128, size=32))
        for _ in range(rows * blocks)
    )
    activations = b"".join(
        _block(float(rng.uniform(0.001, 0.05)), rng.integers(-127, 128, size=32))
    ...
    candidate = (ctypes.c_float * (tokens * rows))()
    tiled = (ctypes.c_float * (tokens * rows))()

    lib.gemm_nt_q8_0_q8_0(a_buf, w_buf, bias, reference, tokens, rows, width)
    lib.gemm_nt_q8_0_q8_0_m2n4(a_buf, w_buf, bias, candidate, tokens, rows, width)

    weight_row_bytes = blocks * 34
    for n0 in range(0, rows, 8):
        tile_rows = min(8, rows - n0)
        lib.gemm_nt_q8_0_q8_0_m2n4_tile(
            a_buf,
