...
static inline void dot_q5_1_q8_1_block_m4_avx2(
        const block_q5_1 *w,
        const block_q8_1 *x0,
        const block_q8_1 *x1,
        const block_q8_1 *x2,
    ...
    const __m128i qhi = _mm_or_si128(_mm_and_si128(_mm_srli_epi16(qpacked, 4), low_mask),
                                     ck_q51_high_bits_16_avx2(qh >> 16));
    const __m256i q5 = _mm256_inserti128_si256(_mm256_castsi128_si256(qlo), qhi, 1);
    const block_q8_1 *rows[4] = {x0, x1, x2, x3};
    const float wd = CK_FP16_TO_FP32(w->d);
    const float wm = CK_FP16_TO_FP32(w->m);
                          ...
                          int K)
{
    ...
    int m = 0;
    for (; m + 3 < M; m += 4) {
        block_q8_1 activation_q8[4][CK_Q51_STACK_Q8_BLOCKS];
        for (int row = 0; row < 4; ++row) {
        ...
        for (int n = 0; n < N; ++n) {
            ...
            for (int block = 0; block < blocks_per_row; ++block) {
                float partial[4];
                dot_q5_1_q8_1_block_m4_avx2(
                    &weight_row[block],
                    &activation_q8[0][block], &activation_q8[1][block],
                    &activation_q8[2][block], &activation_q8[3][block], partial);
                sums[0] += partial[0];
                sums[1] += partial[1];
                sums[2] += partial[2];
                sums[3] += partial[3];
            ...
            C[(size_t)(m + 0) * (size_t)N + n] = sums[0] + add;
            C[(size_t)(m + 1) * (size_t)N + n] = sums[1] + add;
            C[(size_t)(m + 2) * (size_t)N + n] = sums[2] + add;
            C[(size_t)(m + 3) * (size_t)N + n] = sums[3] + add;
        }
    }
